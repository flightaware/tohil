...
tclListObjToPyDictObject(Tcl_Interp *interp, Tcl_Obj *inputObj)
{
    ...
    for (int i = 0; i < count; i += 2) {
        ...
        PyObject *pKey = Py_BuildValue("s", key);
        PyObject *pVal = Py_BuildValue("s", val);
        if (pKey == NULL || pVal == NULL || PyDict_SetItem(pdict, pKey, pVal) < 0) {
            Py_XDECREF(pKey);
            Py_XDECREF(pVal);
            Tcl_DStringFree(&kds);
            Tcl_DStringFree(&vds);
...
tclListObjToPyDictTclObjects(Tcl_Interp *interp, Tcl_Obj *inputObj)
{
    ...
    for (int i = 0; i < count; i += 2) {
        ...
        char *key = tohil_TclObjToUTF8DString(interp, list[i], &kds);
        PyObject *pKey = Py_BuildValue("s", key);
        PyObject *pVal = TohilTclObj_FromTclObj(interp, list[i + 1]);
        if (pKey == NULL || pVal == NULL || PyDict_SetItem(pdict, pKey, pVal) < 0) {
            Py_XDECREF(pKey);
            Py_XDECREF(pVal);
            Tcl_DStringFree(&kds);
...
static PyObject *
Tohil_TD_iter_repr(PyObject *dv)
{
    PyObject *seq;
...
DLLEXPORT int tohil_Init(Tcl_Interp *interp);

DLLEXPORT int tohil_Init(Tcl_Interp *interp)
{
    return Tohil_Init(interp);
}
